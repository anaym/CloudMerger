...
namespace YandexFileHosting
{
    public class YandexService : IService
    {
        ...
        {
            var start = new Uri(string.Format(Resources.StartAuthenticationUrlFormat, Resources.ApplicationToken));
        ...
        public IFileHosting GetFileHostingFor(OAuthCredentials credentials)
        {
            if (credentials.Service.ToLower() != Name.ToLower())
                throw new ArgumentException($"Credentials({credentials.Service}) is not valid for this service ({Name})");

            return new YandexFileHosting(credentials.Token, credentials.Login ?? credentials.Token.Substring(0, 8));
        }

        public override string ToString() => Name;

        private readonly Regex accessTokenRegex= new Regex(@"access_token=(?<token>[a-zA-Z0-9-]+?)&");
